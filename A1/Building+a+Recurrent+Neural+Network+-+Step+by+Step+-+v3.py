...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    y_pr=np.sum(yt_pred,axis=0)
    yt_pred=np.divide(yt_pred,y_pr)
    #yt_pred=softmax(yt_pred)
    ### END CODE HERE ###
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    #yt_pred = np.exp(a_next)
    #yt_p=np.sum(yt_pred,axis=0)
    #yt_pred=np.divide(yt_pred,yt_p)
...
def lstm_forward(x, a0, parameters):
    ...
    act = np.zeros((n_a,m,T_x))
    
    cnt = np.zeros((n_a,m,T_x))
    y_o = np.zeros((n_y,m,T_x))
...
def rnn_cell_backward(da_next, cache):
    ...
    dtanh = np.multiply(da_next,1-a_next**2)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dot = np.multiply(da_next,np.tanh(c_next))
    dcct = np.multiply(np.multiply(da_next,ot),np.multiply(1-np.tanh(c_next)**2,it))+np.multiply(dc_next,it)
    dit =np.multiply(np.multiply(da_next,ot),np.multiply(1-np.tanh(c_next)**2,cct))+np.multiply(dc_next,cct)
    dft = np.multiply(np.multiply(da_next,ot),np.multiply(1-np.tanh(c_next)**2,c_prev))+np.multiply(dc_next,c_prev)
    
    # Code equations (7) to (10) (≈4 lines)
    dit = np.multiply(dit,it*(1-it))
    dft = np.multiply(dft,ft*(1-ft))
    dot = np.multiply(dot,ot*(1-ot))
    dcct = np.multiply(dcct,1-cct**2)
    ...
    da_prev = np.dot(np.transpose(parameters['Wf'])[: n_a,:],dft)+np.dot(np.transpose(parameters['Wi'])[:n_a,:],dit)+np.dot(np.transpose(parameters['Wo'])[:n_a,:],dot)+np.dot(np.transpose(parameters['Wc'])[:n_a,:],dcct)
                      
    dc_prev =np.multiply(dc_next,ft)+np.multiply(da_next*ot*ft,1-np.tanh(c_next)**2)
    dxt = np.dot(np.transpose(parameters['Wf'])[n_a:,:],dft)+np.dot(np.transpose(parameters['Wi'])[n_a:,:],dit)+np.dot(np.transpose(parameters['Wo'])[n_a:,:],dot)+np.dot(np.transpose(parameters['Wc'])[n_a:,:],dcct)
